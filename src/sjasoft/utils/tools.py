...
async def a_set_and(fn, values):
    current = await fn(values[0])
    if not current:
        return current
    for v in values[1:]:
        current_v = await fn(v)
        current = current & current_v
        if not current:
            break
    return current
...
async def a_set_or(fun, items):
    cos = [fun(item) for item in items]
    sets = await asyncio.gather(cos)
...
def command_output(command):
    standard_pipes = dict(stdout=sub.PIPE, stdin=sub.PIPE, stderr=sub.PIPE)
...
def gensym(object):
    trial = "hash_%X" % random.getrandbits(16)  # noqa: F821
...
def tree_order(
    hierarchy, sequence, h_extractor=identity_function, s_extractor=identity_function
):
    """
